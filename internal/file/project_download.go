...
func NewProjectDownload(projectID string, user model.UserSchema, ddirsStore *store.DatadirsStore) *ProjectDownload {
	return &ProjectDownload{ProjectID: projectID, User: user, ddirsStore: ddirsStore}
}
...
func (d *ProjectDownload) CreateProjectDownloadDirectory(basePath string) error {
	...
	ddirs, err := d.ddirsStore.GetDatadirsForProject(d.ProjectID, d.User.ID)
	if err != nil {
...
func (d *ProjectDownload) dirsToCreate(basePath string, ddirs []model.DatadirEntryModel) []string {
	var dirs []string
